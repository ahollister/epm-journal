...
export const useOnboardingStore = create<OnboardingState>((set, get) => ({
  ...
  next: () => {
    ...
    if (nextStage) {
      set({ stage: nextStage, subStep: 0, error: null });
    }
  },
  ...
  goToCharacteristicRating: (id) => {
    const subStep = get().characteristics.findIndex(
      (characteristic) => characteristic.id === id,
    );

    if (subStep >= 0) {
      set({ stage: 'rating', subStep, error: null });
    }
  },
...
export type { Stage } from '@/domain/onboarding/stages';
