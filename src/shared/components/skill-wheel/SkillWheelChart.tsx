...
const WEDGE_OPACITY = 0.5;
const PRESSED_WEDGE_OPACITY = 0.75;
    ...
    <View style={styles.container}>
      <View style={styles.chartWrapper}>
        ...
        >
          ...
          {/* The full-radius paths make short wedges easy to tap. They are
              transparent and rendered before the visible wedges. */}
          {interactive
            ? chartItems.map(({ characteristic, endAngle, index, startAngle }) => (
                <Path
                  key={`hit-wedge-${characteristic.id}-${index}`}
                  d={hitWedgePath(CENTER, CENTER, startAngle, endAngle)}
                  fill="transparent"
                  stroke="transparent"
                  onPressIn={() => setPressedId(characteristic.id)}
                  onPressOut={() => setPressedId(null)}
                  onPress={() => onWedgeTap?.(characteristic.id)}
                />
              ))
            : null}
          ...
          {chartItems.map(
            ({ characteristic, colour, endAngle, hasScore, index, outerRadius, startAngle }) => {
              ...
              const fillOpacity =
                hasScore && pressedId === characteristic.id
                  ? PRESSED_WEDGE_OPACITY
                  : hasScore
                    ? WEDGE_OPACITY
                    : MISSING_SCORE_OPACITY;
              ...
              return (
                <Path
                  ...
                  // The overlay handles the part outside a short wedge. Keep
                  // the same handlers on the visible path because SVG hit
                  // testing selects the topmost path where the two overlap.
                  onPressIn={interactive ? () => setPressedId(characteristic.id) : undefined}
                  onPressOut={interactive ? () => setPressedId(null) : undefined}
                  onPress={interactive ? () => onWedgeTap?.(characteristic.id) : undefined}
                />
              );
            },
          )}
