...
describe('threeListsComplete', () => {
  it.each([
    ...
    [
      'who at maximum boundary',
      makeLists({
        who: Array.from({ length: 10 }, (_, i) => `who-${i}`),
        why: Object.fromEntries(
          Array.from({ length: 10 }, (_, i) => [`who-${i}`, ['reason']]),
        ),
      }),
    ],
  ...
  it.each([
    ...
    [
      'a who entry has no why entries',
      makeLists({
        why: { one: [], two: ['reason'], three: ['reason'], four: ['reason'], five: ['reason'] },
      }),
    ],
    [
      'a who entry is missing from why',
      makeLists({ why: { one: ['reason'], two: ['reason'], three: ['reason'], four: ['reason'] } }),
    ],
...
describe('canAdvance', () => {
  ...
  it.each([
    ...
    [
      'invalid three lists',
      makeState({ threeLists: makeLists({ improvements: ['one', 'two'] }) }),
      false,
    ],
