...
**Update (2026-05-15): Stage 2 WhoList (sub-step 1 of Three Lists) implemented.** Lives at `src/features/onboarding/stages/WhoList.tsx`. Local 3-min `useWhoTimer` hook: `useState(180)` + `useEffect`/`setInterval` (1s tick), component-local only — not in the store, not in domain, not persisted (the timer is a psychological nudge, not a hard gate). At 0:00, timer text changes colour and shows "Time's up — keep going if you need to." Input stays fully functional. TextInput with placeholder "Add a musician…" + "Add" button, Return-key submits. Names stored in `store.threeLists.who`. Min 5 / max 10 with inline guidance. Trim on submit; no duplicate detection. Prompts via expandable "Need ideas?" section. "Next" advances subStep to WhyList; data preserved across back/forward. Remaining Stage 2 sub-steps (WhyList, ImprovementsList) and Stages 3–6 remain pending.


**Update (2026-05-15): Stage 2 WhyList (sub-step 2 of Three Lists) implemented.** Lives at `src/features/onboarding/stages/WhyList.tsx`. Two-column responsive layout: left column = Who names (read-only), right column = quality `TextInput` areas with unlimited "Add another" per Who. Completion gate: every Who must have ≥1 non-empty Why entry; rows with 0 qualities get a warning icon/highlight; "Next" disabled until all rows pass. Static prompts per FR3.3 always visible. Data stored in `store.threeLists.why` as `Record<string, string[]>` keyed by who name (duplicate names share key per documented V1 behaviour). Whitespace-only entries rejected by both UI and store guard. "Next" → ImprovementsList (subStep 3); "Back" → WhoList (subStep 1, data preserved). Remaining Stage 2 sub-step (ImprovementsList) and Stages 3–6 remain pending.
...
**Update (2026-05-15): Stages 3A, 3B, 4, 5, 6 — all remaining onboarding stages implemented.** The seven-task sequential queue (Stage 3A Characteristic Creation, 3B Characteristic Review/Edit, 4 Self-Rating, 5 Confirmation/Wheel Preview, 6 Goal Suggestion, Progress empty state, Progress skill wheel rendering) all landed on separate feature branches and have been consolidated on `feature/skillwheel-stage-3-4-5`. The full 7-stage onboarding flow is now implemented end-to-end: Intro → Three Lists (Who/Why/Improvements) → Characteristic Creation → Characteristic Review → Self-Rating → Confirmation → Goal Suggestion → Completion. Progress tab renders the shared `SkillWheelChart` with persisted baseline data when onboarding is complete, or an empty state CTA when no baseline exists. **All seven onboarding stages are now implemented.**
